...
rockets_description = cop.elements.Property(
    'rockets_description', 'Data about the rockets built by SpaceX')
...
for rocket in rockets:
    ...
    short_description = cop.elements.Property(
        'description', shorten_description(rocket['description']))
...
data.add(cop.elements.Property('dragons_description',
...
for dragon in dragons:
    ...
    short_description = cop.elements.Property(
        'description', shorten_description(dragon['description']))
    collec.add(short_description)  # Overwrites the current description
...
data.add(cop.elements.Property('launch_pads_description',
...
for launch_pad in launch_pads:
    ...
    img = cop.elements.Image.from_url(
        'image', launch_pad['images']['large'][0])
    img.max_height = IMAGE_MAX_HEIGHT
    img.max_width = IMAGE_MAX_WIDTH
    collec.add(img)

    short_description = cop.elements.Property(
        'details', shorten_description(launch_pad['details']))
    collec.add(short_description)  # Overwrites the current description
...
data.add(cop.elements.Property('landing_pads_description',
...
for landing_pad in landing_pads:
    ...
    img = cop.elements.Image.from_url(
        'image', landing_pad['images']['large'][0])
    img.max_height = IMAGE_MAX_HEIGHT
    img.max_width = IMAGE_MAX_WIDTH
    collec.add(img)
    ...
    short_description = cop.elements.Property(
        'details', shorten_description(landing_pad['details']))
    collec.add(short_description)  # Overwrites the current description
...
data.add(cop.elements.Property('ships_description',
...
printjob = cop.PrintJob(
    ...
    template=cop.Resource.from_local_file(
        './examples/spacex_example/spacex_template.pptx'),  # For pptx
    # template=cop.Resource.from_local_file(
    #     './examples/spacex_example/spacex_template.xlsx'),  # For xlsx
    # template=cop.Resource.from_local_file(
    #     './examples/spacex_example/spacex_template.docx'),  # For docx
)
