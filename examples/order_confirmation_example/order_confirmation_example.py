...
image = cop.elements.Image.from_base64(
    "image",
...
template = cop.Template.from_local_file("template.docx")
config = cop.config.OutputConfig("pdf")  # Optional
print_job = cop.PrintJob(data, server, template, config)

# Execute print job and save response to output file
try:
